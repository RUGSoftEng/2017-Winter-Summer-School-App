...
*/
(function (s, p){
	"use strict";let c = "BUTTON A INPUT TEXTAREA SELECT DETAILS SUMMARY".split(" "), h = function (a, b){
		if (-1 !== b.indexOf(a[0].nodeName)) return !0;
	};p.module("ngAria", ["ng"]).info({ angularVersion: "1.6.5" }).provider("$aria", function (){
		function a (a, c, n, k){
			return function (d, f, e){
				const g = e.$normalize(c);!b[g] || h(f, n) || e[g] || d.$watch(e[a], function (a){
					a = k ? !a : !!a;f.attr(c, a);
				});
			};
		} var b = {
			...
			restrict: "A", require: "ngModel", priority: 200, compile: function (c, k){
				const d = m(k, c);return {
					post: function (f, e, g, c){
						...
						} const n = b("tabindex", "tabindex", e, !1);switch (d){
						case "radio":case "checkbox":l(d, e) && e.attr("role", d);b("aria-checked", "ariaChecked", e, !1) && f.$watch(k,
							"radio" === d ? h : m);n && e.attr("tabindex", 0);break;case "range":l(d, e) && e.attr("role", "slider");if (a.config("ariaValue")){
							let p = !e.attr("aria-valuemin") && (g.hasOwnProperty("min") || g.hasOwnProperty("ngMin")), q = !e.attr("aria-valuemax") && (g.hasOwnProperty("max") || g.hasOwnProperty("ngMax")), r = !e.attr("aria-valuenow");p && g.$observe("min", function (a){
...
b.attr("aria-live", "assertive");
		...
		return {
			restrict: "A", compile: function (l, m){
				const n = b(m.ngClick);return function (b, d, f){
					if (!h(d, c) && (a.config("bindRoleForClick") && !d.attr("role") && d.attr("role", "button"), a.config("tabindex") && !d.attr("tabindex") && d.attr("tabindex", 0), a.config("bindKeydown") && !f.ngKeydown && !f.ngKeypress && !f.ngKeyup))d.on("keydown", function (a){
						function c (){
							n(b, { $event: a });
						} const d = a.which || a.keyCode;32 !== d && 13 !== d || b.$apply(c);
					});
				};
			} 
		};
